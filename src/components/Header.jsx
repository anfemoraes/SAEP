...
export function Header({ onNavigate, usuarioLogado, setUsuarioLogado, telaAtual }) {
    ...
    const [senha, setSenha] = useState(''); // CORRIGIDO: setSenha com 's' minúsculo
            ...
            <nav className="header-nav" style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                {estaLogado && (
                    <>
                        ...
                        <button onClick={() => onNavigate('rascunhos')} style={estiloBotaoNav('rascunhos')}>Meus Rascunhos</button>
                        <button onClick={() => onNavigate('dashboard')} style={estiloBotaoNav('dashboard')}>Dashboard</button> {/* CORRIGIDO: mudou de 'andamento' para 'dashboard' */}
                    </>
                )}
                {ehComiteOuAdmin && (
