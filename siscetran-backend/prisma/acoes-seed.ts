...
/**
 * Conversor de "prazo" do formato do frontend (acoes_data.js) para o enum do Prisma.
 * Frontend usa strings como "Curto", "Médio", "Longo".
 */
export function converterPrazo(prazoFrontend: string): Prazo {
  const normalizado = prazoFrontend
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .trim()
    .toUpperCase();

  if (normalizado.includes('CURTO')) return Prazo.CURTO_PRAZO;
  if (normalizado.includes('MEDIO')) return Prazo.MEDIO_PRAZO;
  if (normalizado.includes('LONGO')) return Prazo.LONGO_PRAZO;

  throw new Error(`Prazo não reconhecido: "${prazoFrontend}"`);
}

export interface AcaoFrontend {
  id: string;
  diretriz: string;
  prazo: string;
  meta: string;
...
/**
 * Converte uma ação no formato do acoes_data.js (frontend) para o formato
 * esperado pelo Prisma (CreateAcaoDto).
 *
 * IMPORTANTE - campos que NÃO existem em acoes_data.js:
 * - `og` e `lae`: o frontend não tem esses códigos/descrições separados do OG e da LAE,
 *   apenas o `id` (ex: "AE 1.1.1.1") e a `diretriz`. Ficam `null` até serem preenchidos
 *   manualmente (ex: pela tela de admin) ou extraídos de outra fonte (planilha original).
 * - `setor`: usamos o valor de `responsavel` como aproximação, já que no frontend esse campo
 *   guarda o nome do setor/comitê responsável (ex: "Secretaria-Executiva", "CTSIST").
 * - `linhaPlanilha`: não é o número da linha da planilha original (não temos essa informação
 *   aqui) — é a posição sequencial da ação dentro do arquivo acoes_data.js, usada apenas
 *   para preservar a ordem de exibição.
 */
export function converterAcaoParaPrisma(acao: AcaoFrontend, indiceSequencial: number) {
  return {
    id: acao.id,
    og: undefined,
    lae: undefined,
    diretriz: acao.diretriz,
    prazo: converterPrazo(acao.prazo),
    setor: acao.responsavel,
    meta: acao.meta,
    indicador: acao.indicador,
    restricoes: undefined,
    linhaPlanilha: indiceSequencial,
    responsavel: acao.responsavel,
    dadosIncompletos: ['og', 'lae'],
  };
}

/**
 * As 147 ações estratégicas do PETRANS, extraídas de acoes_data.js.
 */
export const acoesFrontend: AcaoFrontend[] = [
  {
    id: "AE 1.1.1.1",
